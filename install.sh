...
# installer/masterr_install.py, not here.

set -e

REPO_URL="${MASTER_REPO_URL:-https://github.com/Krish-Kamani/MasterR.git}"
...
main() {
	...
	[ "$(uname -s)" = Linux ] || die "MasterR only installs on Linux"
	...
	# If running directly inside a local checkout of MasterR
	SCRIPT_DIR="$(cd "$(dirname "$0")" 2>/dev/null && pwd)"
	if [ -f "$SCRIPT_DIR/installer/masterr_install.py" ] && [ -d "$SCRIPT_DIR/configs" ]; then
		exec python3 "$SCRIPT_DIR/installer/masterr_install.py" --source "$SCRIPT_DIR/configs" "$@"
	fi

	fetch
	exec python3 "$DIR/installer/masterr_install.py" --source "$DIR/configs" "$@"
}
