...
GRUB_EXCLUDED = ["grub/install-torii.sh", "grub/probe-sda4.sh", "grub/10_masterr"]

# User-owned config files a re-run must never reset: the same protected set the
# update engine three-way merges (hand-mirrored from masterr-update.py, which
...
def detect_existing(config_root=CONFIG_ROOT):
    ...
    and, if so, whether MasterR put it there (carries our marker) or it is a
...
def _fastfetch_palette():
    ...
    """
    cache = Path.home() / ".cache" / "masterr" / "colors.json"
    if cache.is_file():
        try:
...
def uninstall(config_root=CONFIG_ROOT, apply=False):
    """
    Remove every MasterR-managed item from ~/.config and put its pristine .bak
    ...
    for name, _src_rel, dest_rel in DEPLOY_SET:
        ...
        if not _is_managed(dest, is_dir):
            actions.append({"item": name, "action": "skip",
                            "reason": "not MasterR-managed", "dest": str(dest)})
            continue
...
def _selftest():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check("hl.monitor" in (root / "hypr" / "modules" / "monitors.lua").read_text(),
        ...
        check((root / "hypr" / "modules" / "monitors.lua.example").read_text().find("hl.monitor") >= 0,
        ...
        check("?~/.cache/masterr/ghostty-colors" in gh,
        ...
        if (root / "hypr" / "ghosttype.lua").exists():
    ...
    spec = importlib.util.spec_from_file_location(
        "_masterr_update", REPO_ROOT / "configs" / "hypr" / "scripts" / "masterr-update.py")
