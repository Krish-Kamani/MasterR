#!/usr/bin/env python3
"""
Package-manager operations for the MasterR multi-distro installer. distro.py
...
def ensure_aur_helper_steps():
    ...
    if aur_helper() is not None:
        return []
    build_dir = shlex.quote(os.path.join(tempfile.gettempdir(), "masterr-yay-build"))
    return [
...
def _write_os_release(fields):
    ...
    fd, path = tempfile.mkstemp(prefix="masterr-osr-")
