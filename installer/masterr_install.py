#!/usr/bin/env python3
"""
The MasterR installer orchestrator: the thin top layer that ties distro
...
def _wizard(args, info, manifest):
    ...
    if info["bootloader"] == "grub":
        grub = tui.confirm("GRUB theme", [
            "Install the MasterR GRUB theme.",
    ...
    if not args.brave:
        bidx = tui.select_one("Brave browser", [
            ("Install Brave", "Brave browser with the matching MasterR theme", True),
...
def _summary_lines(info, choices, plan, args, do_pkgs):
    ...
    if choices["brave"]:
        lines.append("Install Brave with the matching MasterR theme.")
    if choices["fish"]:
        lines.append("Set fish as your login shell.")
    if _is_update(info):
        lines.append("Update the MasterR config; your Settings are kept.")
    else:
        lines.append("Back up and deploy the MasterR config.")
    return lines


def _is_update(info):
    """
    True when this run lands on top of an earlier MasterR deploy, spotted by the
...
def deploy_brave_theme(source, dry):
    """
    Copy the bundled Brave theme into ~/.config/masterr so the user can point
    ...
    """
    dest_show = "~/.config/masterr/brave-theme"
    if dry:
...
def _seed_update_baseline(source, config_root, dry):
    ...
    try:
        ...
        return True, ""
    engine = Path(config_root) / "hypr" / "scripts" / "masterr-update.py"
...
def _report(plan, failures, notes, info, choices, args, do_pkgs, dry):
    ...
    if choices["brave"]:
        steps.append(("brave theme",
                      "brave://settings/appearance, load ~/.config/masterr/brave-theme"))
    ...
    title = "Dry run complete" if dry else "MasterR is in"
...
def run(args):
    ...
    if _is_update(info):
        config_label = "MasterR (this run updates it, your Settings are kept)"
    elif has_config:
    ...
    try:
        ...
        # k2. put the masterr control CLI on PATH now that the script is deployed.
        ok, detail, linked = link_masterr_cli(dry)
        record(ok, detail, "Link masterr CLI",
               "Symlink ~/.local/bin/masterr to ~/.config/hypr/scripts/masterr yourself.")
        if linked:
        ...
        if choices["brave"]:
            ...
            record(ok, detail, "Deploy Brave theme",
                   "Copy configs/brave-theme to ~/.config/masterr/brave-theme yourself.")
    finally:
        if keepalive_stop:
            keepalive_stop()
...
def main():
    parser = argparse.ArgumentParser(
        description="Install the MasterR Hyprland rice across distro families.")
    ...
    parser.add_argument("--brave", action="store_true",
                        help="Preselect Brave plus its MasterR theme")
