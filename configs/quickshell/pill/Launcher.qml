...
PillSurface {
    ...
    function appimageSlug(entry) {
        return entry && entry.id && entry.id.indexOf("masterr-") === 0 ? entry.id.substring(8) : "";
    }
    ...
    readonly property string usageFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/masterr/launcher-usage.json"
    ...
    ListView {
        ...
        delegate: Item {
            ...
            readonly property bool isAppImage: entry && entry.id && entry.id.indexOf("masterr-") === 0
            ...
            Item {
                ...
                Image {
                    ...
                    source: {
                        ...
                        // Handle absolute paths for ANY app, not just AppImages
                        if (ic.indexOf("/") === 0)
                            return "file://" + ic;
