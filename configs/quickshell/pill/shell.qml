...
ShellRoot {
    ...
    readonly property bool anyPillExpanded: expandedCount > 0 || openSurface.length > 0 || peekMon.length > 0
    signal unpinAll()
    ...
    Process {
        id: updatedToast
        command: ["sh", "-c",
            "m=\"${XDG_STATE_HOME:-$HOME/.local/state}/masterr/updated\"; [ -f \"$m\" ] || exit 0; "
            + "b=$(cat \"$m\"); rm -f \"$m\"; "
            + "gdbus call --session --dest org.freedesktop.Notifications "
            + "--object-path /org/freedesktop/Notifications "
            + "--method org.freedesktop.Notifications.Notify "
            + "MasterR 0 '' 'MasterR updated' \"$b\" '[]' '{}' 5000 >/dev/null 2>&1"]
    }
    ...
    Process {
        ...
        command: ["systemd-inhibit", "--what=idle:sleep", "--who=MasterR",
    ...
    function isAnyOpen() {
        if (root.openSurface.length > 0) return true;
        if (root.peekMon.length > 0) return true;
        if (ScreenRec.quickChoosing || ScreenRec.quickScreenChoosing) return true;
        if (root.anyPillExpanded) return true;
        return false;
    }
    ...
    function peek(mon) {
        if (!mon || mon.length === 0)
            mon = Hyprland.focusedMonitor ? Hyprland.focusedMonitor.name : "";
        if (!mon || mon.length === 0)
            mon = (Quickshell.screens.length > 0 && Quickshell.screens[0]) ? Quickshell.screens[0].name : "";
    ...
    IpcHandler {
        ...
        function chatbot(mon: string): void { root.toggleSurface(mon, "chatbot"); }
        function chat(mon: string): void { root.toggleSurface(mon, "chatbot"); }
        function ai(mon: string): void { root.toggleSurface(mon, "chatbot"); }
        function finder(mon: string): void { root.toggleSurface(mon, "finder"); }
        function files(mon: string): void { root.toggleSurface(mon, "finder"); }
        function packages(mon: string): void { root.toggleSurface(mon, "packages"); }
        function pkg(mon: string): void { root.toggleSurface(mon, "packages"); }
        ...
        function overview(mon: string): void { root.toggleSurface(mon, "overview"); }
        ...
        function dock(mon: string): void { Flags.dockEnable = !Flags.dockEnable; }
        function dockPin(mon: string): void { Flags.dockPinned = !Flags.dockPinned; }
        function dockToggle(mon: string): void { Flags.dockEnable = !Flags.dockEnable; }
    ...
    Variants {
        ...
        Dock {
            ...
            onRequestLauncher: root.toggleSurface(modelData ? modelData.name : "", "launcher")
        }
    }
    ...
    Variants {
        ...
        PanelWindow {
            ...
            FocusScope {
                ...
                property bool returnHeld: false
                property bool returnMovedWindow: false
                ...
                Keys.onUpPressed: (e) => {
                    if (pill.overviewOpen) {
                        if (focusScope.returnHeld || (e.modifiers & Qt.ShiftModifier)) {
                            pill.overviewMoveWindow("up");
                            focusScope.returnMovedWindow = true;
                        } else {
                            pill.overviewMove("up");
                        }
                        e.accepted = true;
                        return;
                    }
                ...
                Keys.onDownPressed: (e) => {
                    if (pill.overviewOpen) {
                        if (focusScope.returnHeld || (e.modifiers & Qt.ShiftModifier)) {
                            pill.overviewMoveWindow("down");
                            focusScope.returnMovedWindow = true;
                        } else {
                            pill.overviewMove("down");
                        }
                        e.accepted = true;
                        return;
                    }
                ...
                Keys.onLeftPressed: (e) => {
                    if (pill.overviewOpen) {
                        if (focusScope.returnHeld || (e.modifiers & Qt.ShiftModifier)) {
                            pill.overviewMoveWindow("left");
                            focusScope.returnMovedWindow = true;
                        } else {
                            pill.overviewMove("left");
                        }
                        e.accepted = true;
                    }
                ...
                Keys.onRightPressed: (e) => {
                    if (pill.overviewOpen) {
                        if (focusScope.returnHeld || (e.modifiers & Qt.ShiftModifier)) {
                            pill.overviewMoveWindow("right");
                            focusScope.returnMovedWindow = true;
                        } else {
                            pill.overviewMove("right");
                        }
                        e.accepted = true;
                    }
                    else if (pill.mixerOpen) { pill.mixerFocusMove(1); e.accepted = true; }
                ...
                Keys.onReturnPressed: (e) => {
                    if (pill.overviewOpen) {
                        if (!e.isAutoRepeat) {
                            focusScope.returnHeld = true;
                            focusScope.returnMovedWindow = false;
                        }
                        e.accepted = true;
                 ...
                 */
                Keys.onPressed: (e) => {
                    if (pill.chatbotOpen && pill.chatbotResize(e)) {
                        e.accepted = true;
                        return;
                    }
                    if (pill.overviewOpen) {
                        if (e.key === Qt.Key_W) {
                            pill.overviewSelectWindow("w");
                            e.accepted = true;
                            return;
                        } else if (e.key === Qt.Key_A) {
                            pill.overviewSelectWindow("a");
                            e.accepted = true;
                            return;
                        } else if (e.key === Qt.Key_S) {
                            pill.overviewSelectWindow("s");
                            e.accepted = true;
                            return;
                        } else if (e.key === Qt.Key_D) {
                            pill.overviewSelectWindow("d");
                            e.accepted = true;
                            return;
                        }
                        if (e.key === Qt.Key_Tab) {
                        ...
                        if (e.key >= Qt.Key_1 && e.key <= Qt.Key_9) {
                            Hyprland.dispatch('hl.dsp.focus({workspace="' + (e.key - Qt.Key_0) + '"})');
                            root.close();
                            e.accepted = true;
                            return;
                ...
                Keys.onReleased: (e) => {
                    if (e.isAutoRepeat)
                        return;
                    if (pill.overviewOpen && (e.key === Qt.Key_Return || e.key === Qt.Key_Enter)) {
                        pill.overviewResetWindow();
                        if (!focusScope.returnMovedWindow) {
                            root.close();
                        }
                        focusScope.returnHeld = false;
                        focusScope.returnMovedWindow = false;
                        e.accepted = true;
                        return;
                    }
                ...
                Pill {
                    ...
                    onExpandedChanged: {
                        if (expanded) root.expandedCount++;
                        else root.expandedCount = Math.max(0, root.expandedCount - 1);
                    }
