...
Item {
    ...
    readonly property bool finderOpen: surface === "finder" || surface === "files"
    readonly property bool packagesOpen: surface === "packages" || surface === "pkg"
    ...
    readonly property bool docksettingsOpen: surface === "docksettings"
    readonly property bool overviewOpen: surface === "overview"
    readonly property bool chatbotOpen: surface === "chatbot" || surface === "chat" || surface === "ai"
    readonly property bool settingsLike: settingsOpen || appearanceOpen || updatesOpen
        || lookOpen || inputOpen || displayOpen || animationOpen || idlelockOpen || fontpickerOpen || docksettingsOpen
    ...
    readonly property real finderW: 400 * s
    readonly property real finderH: 340 * s
    readonly property real packagesW: 420 * s
    readonly property real packagesH: 360 * s
    ...
    readonly property real docksettingsW: 392 * s
    readonly property real overviewW: 1060 * s
    readonly property real overviewH: 260 * s
    readonly property real chatbotW: (Flags.chatbotWidth > 0 ? Flags.chatbotWidth : 500) * s
    readonly property real chatbotH: (Flags.chatbotHeight > 0 ? Flags.chatbotHeight : 540) * s
    ...
    readonly property var surfaces: ({
        ...
        finder:    { size: () => { surfaceItem(ldFinder); return Qt.size(finderW, finderH); }, ame: () => surfaceItem(ldFinder) },
        files:     { size: () => { surfaceItem(ldFinder); return Qt.size(finderW, finderH); }, ame: () => surfaceItem(ldFinder) },
        packages:  { size: () => { surfaceItem(ldPackages); return Qt.size(packagesW, packagesH); }, ame: () => surfaceItem(ldPackages) },
        pkg:       { size: () => { surfaceItem(ldPackages); return Qt.size(packagesW, packagesH); }, ame: () => surfaceItem(ldPackages) },
        ...
        docksettings: { size: () => Qt.size(docksettingsW, surfaceItem(ldDockSettings).implicitHeight + 29 * s), ame: () => surfaceItem(ldDockSettings) },
        overview:     { size: () => { surfaceItem(ldOverview); return Qt.size(overviewW, overviewH); }, ame: () => null },
        chatbot:      { size: () => { surfaceItem(ldChatbot); return Qt.size(chatbotW, chatbotH); }, ame: () => surfaceItem(ldChatbot) },
        chat:         { size: () => { surfaceItem(ldChatbot); return Qt.size(chatbotW, chatbotH); }, ame: () => surfaceItem(ldChatbot) },
        ai:           { size: () => { surfaceItem(ldChatbot); return Qt.size(chatbotW, chatbotH); }, ame: () => surfaceItem(ldChatbot) }
    })
    ...
    function chatbotResize(e) {
        return (pill.chatbotOpen && ldChatbot.item) ? ldChatbot.item.handleResizeKey(e) : false;
    }
    ...
    function rowNavSurface() {
        ...
        if (pill.docksettingsOpen)
            return ldDockSettings.item;
        return null;
    }
    ...
    function overviewMove(dir) {
        if (pill.overviewOpen && ldOverview.item)
            ldOverview.item.moveFocus(dir);
    }

    function overviewMoveWindow(dir) {
        if (pill.overviewOpen && ldOverview.item)
            ldOverview.item.moveActiveWindow(dir);
    }

    function overviewResetWindow() {
        if (pill.overviewOpen && ldOverview.item)
            ldOverview.item.resetLockedWindow();
    }

    function overviewSelectWindow(key) {
        if (pill.overviewOpen && ldOverview.item)
            ldOverview.item.selectWindowKey(key);
    }
    ...
    Loader {
        ...
        active: false
        anchors.fill: parent
        sourceComponent: Finder {
            s: pill.s
            open: pill.finderOpen
            morphCloseness: pill.morphCloseness
            onRequestClose: pill.requestClose()
        }
    }
    ...
    Loader {
        ...
        active: false
        anchors.fill: parent
        sourceComponent: OverviewSurface {
            s: pill.s
            screenName: pill.screenName
            open: pill.overviewOpen
            morphCloseness: pill.morphCloseness
            onRequestClose: pill.requestClose()
        }
    }
    ...
    Loader {
        ...
        active: false
        anchors.fill: parent
        sourceComponent: Chatbot {
            s: pill.s
            open: pill.chatbotOpen
            morphCloseness: pill.morphCloseness
            onRequestClose: pill.requestClose()
        }
    }
