...
function keyName(key) {
    ...
    if (key >= 0x61 && key <= 0x7a) return String.fromCharCode(key - 32);
...
function formatPretty(combo) {
    if (!combo) return "";
    return combo.replace(/mouse_up/g, "Scroll ↑")
                .replace(/mouse_down/g, "Scroll ↓")
                .replace(/mouse:272/g, "LMB")
                .replace(/mouse:273/g, "RMB")
                .replace(/\bslash\b/g, "/")
                .replace(/\bperiod\b/g, ".")
                .replace(/\bcomma\b/g, ",")
                .replace(/\bminus\b/g, "-")
