...
/**
 * 更 UPDATES sub-surface: a terminal-free face for the MasterR update engine. It
 * never touches git itself; it shells out to the python engine at
 * ~/.config/hypr/scripts/masterr-update.py, which prints one JSON object, and
...
SettingsSurface {
    ...
    readonly property string engine: Quickshell.env("HOME") + "/.config/hypr/scripts/masterr-update.py"
    ...
    FileView {
        id: manifestFile
        path: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/masterr/update.json"
        watchChanges: true
        printErrors: false
    ...
    Process {
        ...
        command: ["sh", "-c",
            "d=\"${XDG_STATE_HOME:-$HOME/.local/state}/masterr\"; mkdir -p \"$d\"; printf '%s' \"$1\" > \"$d/updated\"",
            "sh", body]
    }
