...
PillSurface {
    ...
    Keys.onLeftPressed: (e) => { root.move(-1); e.accepted = true; }
    Keys.onRightPressed: (e) => { root.move(1); e.accepted = true; }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            prevFetch.command = ["bash", "-c",
                "f=\"/tmp/masterr-wp-preview-$(printf %s \"$1\" | md5sum | cut -d' ' -f1).webm\"; [ -s \"$f\" ] || curl -fsL --max-time 25 -A 'Mozilla/5.0' -o \"$f\" \"$1\" || { rm -f \"$f\"; exit 1; }; printf %s \"$f\"",
