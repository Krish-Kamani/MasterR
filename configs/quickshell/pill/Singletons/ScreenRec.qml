 ...
 * `$XDG_CACHE_HOME/masterr/rec-thumbs`, skipping clips already cached) and only
...
Singleton {
    ...
    readonly property string thumbDir: (Quickshell.env("XDG_CACHE_HOME") || (home + "/.cache")) + "/masterr/rec-thumbs/"
    ...
    Process {
        ...
        onExited: function(exitCode) {
            ...
            if (exitCode !== 0) {
                ...
                failProc.command = ["notify-send", "-a", "MasterR", "-u", "critical",
    ...
    Process {
        id: savedProc
        command: ["notify-send", "-a", "MasterR", "Recording saved",
