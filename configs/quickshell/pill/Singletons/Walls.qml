 ...
 * to the masterr-wallpaper-dir state file on its last run, then
 * ~/MasterR/wallpapers for a first boot before wallpaper.sh init has run.
 *
...
Singleton {
    ...
    readonly property string wpDir: Flags.wallpaperDir.length > 0 ? Flags.wallpaperDir
        : (resolvedDir.length > 0 ? resolvedDir : Quickshell.env("HOME") + "/MasterR/wallpapers")
    ...
    readonly property string stateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/masterr-wallpaper"
    readonly property string dirStateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/masterr-wallpaper-dir"
