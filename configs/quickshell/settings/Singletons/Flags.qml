 ...
 * change, so every MasterR daemon (pill, lock) reads and writes the same
...
Singleton {
    ...
    property alias dockEnable: adapter.dockEnable
    property alias dockPinned: adapter.dockPinned
    property alias dockPinMode: adapter.dockPinMode
    property alias dockHoverToReveal: adapter.dockHoverToReveal
    property alias dockHeight: adapter.dockHeight
    property alias dockBottomGap: adapter.dockBottomGap
    property alias dockWindowGap: adapter.dockWindowGap
    property alias dockPinnedApps: adapter.dockPinnedApps
    property alias dockMonochromeIcons: adapter.dockMonochromeIcons
    property alias powerProfile: adapter.powerProfile
    property alias fanSpeed: adapter.fanSpeed
    property alias chatbotWidth: adapter.chatbotWidth
    property alias chatbotHeight: adapter.chatbotHeight
    property alias smoothWorkspaceScroll: adapter.smoothWorkspaceScroll
    property alias aiConfirmCommand: adapter.aiConfirmCommand
    property alias aiConfirmFile: adapter.aiConfirmFile
    property alias lockscreenShowBattery: adapter.lockscreenShowBattery

    FileView {
        id: file
        path: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/masterr/flags.json"
        blockLoading: true
        watchChanges: true
        printErrors: false
        ...
        JsonAdapter {
            ...
            /** Explicit wallpaper folder override. Empty means autodetect: the dir wallpaper.sh last resolved (masterr-wallpaper-dir state file), then ~/MasterR/wallpapers. Lives in user state so an in-app update never clobbers a custom folder. */
            ...
            property bool dockEnable: true
            property bool dockPinned: true
            property string dockPinMode: "desktop" // "always", "desktop", "autohide"
            property bool dockHoverToReveal: true
            property real dockHeight: 52
            property real dockBottomGap: 8
            property real dockWindowGap: 8
            property var dockPinnedApps: ["org.kde.dolphin", "ghostty", "zen", "kitty", "firefox"]
            property bool dockMonochromeIcons: false
            property string powerProfile: "balanced"
            property string fanSpeed: "auto"
            property real chatbotWidth: 500
            property real chatbotHeight: 540
            property bool smoothWorkspaceScroll: true
            property bool aiConfirmCommand: true
            property bool aiConfirmFile: true
            property bool lockscreenShowBattery: true
        }
    }
}
