#!/usr/bin/env python3
"""
Update engine for the MasterR rice. The in-app Settings updater shells out to
...
DEFAULT_REMOTE = "https://github.com/Krish-Kamani/MasterR.git"
...
def data_dir():
    ...
    return Path(base) / "masterr-update"


def manifest_path():
    base = os.environ.get("XDG_STATE_HOME") or str(Path.home() / ".local" / "state")
    return Path(base) / "masterr" / "update.json"
...
def backup_protected(config_root):
    ...
    dest_root = data_dir().parent / "masterr-update-backup" / stamp
    made = None
...
#
# A MasterR update can introduce a new package the rice now needs (cava did once).
